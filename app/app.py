...
def register():
    ...
    if  form.validate_on_submit():
        new_user = User(username=form.username.data, password=form.password.data, gender=form.gender.data, full_name=form.full_name.data, email=form.email.data)
...
class CheckoutForm(FlaskForm):
    first_name = StringField('First Name', render_kw={"placeholder": "e.g. Jack"})
    last_name = StringField('Last Name', render_kw={"placeholder": "e.g. Ingoff"})
    phone_number = StringField('Phone Number', render_kw={"placeholder": "e.g. 912345678"})
    email = StringField('Email', render_kw={"placeholder": "e.g. email@ua.pt"})
    address = StringField('Address', render_kw={"placeholder": "e.g. Cristiano Ronaldo Street 7"})
    city = StringField('City')
    country = StringField('Country')
    payment_method = SelectField('Payment Method', choices=[('Credit Card', 'Credit Card'), ('Multibanco', 'Multibanco'), ('Paypal', 'Paypal'), ('MBWay', 'MBWay')])
    card_number = StringField('Card Number', render_kw={"placeholder": "XXXX XXXX XXXX XXXX"})
    expiration_date = StringField('Card Expiration', render_kw={"placeholder": "MM/YY"})
    cvv = StringField('Card CVV', render_kw={"placeholder": "XXX"})
    paypal_email = StringField('PayPal Email', render_kw={"placeholder": "paypal@email.com"})
    mbway_phone_number = StringField('Phone Number', render_kw={"placeholder": "e.g. 912345678"})
